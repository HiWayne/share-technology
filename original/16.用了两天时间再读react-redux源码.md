...
首先是设置 subscription 的 onStateChange，它会在更新触发时调用，它这里希望将来调用的是`subscription.notifyNestedSubs`，`subscription.notifyNestedSubs`会触发这个 subscription 收集的所有子订阅。
...
function Provider<A extends Action = AnyAction>({
  ...
  // 生成了一个透传的对象，包含redux store、subscription实例、SSR时可能用到的函数
  const contextValue = useMemo(() => {
    const subscription = createSubscription(store);
    return {
      store,
